...
调用构造函数时，会为实例对象添加一个内部属性`[[Protoype]]`，这个属性指向构造函数最初的原型对象，也就是`prototype`属性指向的那个对象。当读取一个对象的属性时，EcmaScript会首先在实例属性中查找，如果找到了对应的属性或方法就会停止查找，然后返回查找结果。如果在实例属性中没有找到，就会继续在对象的`[[Prototype]]`属性指向的那个对象中查找。在实例上的属性会屏蔽原型中的同名属性。
